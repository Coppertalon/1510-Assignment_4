from unittest import TestCase
import location_callers
from unittest.mock import patch
from location_descriptions import location_city_4
import io

class Test(TestCase):

    @patch('builtins.input', side_effect=['1'])
    def test_location_city_4_not_done_move(self, mock_input):
        maps = {"map_visual": [["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"]],
                "map_locations": [["3", "3", "3", "y", "4"],
                                  ["c", "c", "3", "y", "y"],
                                  ["2", "2", "c", "3", "3"],
                                  ["p", "2", "2", "c", "3"],
                                  ["1", "p", "2", "c", "3"]]}
        player_character = {"name": "test",
                            "player_position": [4, 0],
                            "health": 3,
                            "level": 2,
                            "exp": 0,
                            "add": 0,
                            "take_away": 0,
                            "re_roll": 0}

        output = location_city_4(maps, player_character, False)
        self.assertEqual(output, (0, True))
        self.assertEqual(maps["map_visual"], [["*", "*", "*", "*", "*"],
                                              ["*", "*", "*", "*", "*"],
                                              ["*", "*", "*", "*", "*"],
                                              ["*", "*", "*", "*", "*"],
                                              ["C", "*", "*", "*", "*"]])
        self.assertEqual(maps["map_locations"], [["3", "3", "3", "y", "4"],
                                                 ["c", "c", "3", "y", "y"],
                                                 ["2", "2", "c", "3", "3"],
                                                 ["p", "2", "2", "c", "3"],
                                                 [location_city_4, "p", "2", "c", "3"]])

    @patch('builtins.input', side_effect=['2'])
    def test_location_city_4_not_done_quit(self, mock_input):
        maps = {"map_visual": [["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"],
                               ["*", "*", "*", "*", "*"]],
                "map_locations": [["3", "3", "3", "y", "4"],
                                  ["c", "c", "3", "y", "y"],
                                  ["2", "2", "c", "3", "3"],
                                  ["p", "2", "2", "c", "3"],
                                  ["1", "p", "2", "c", "3"]]}
        player_character = {"name": "test",
                            "player_position": [4, 0],
                            "health": 3,
                            "level": 2,
                            "exp": 0,
                            "add": 0,
                            "take_away": 0,
                            "re_roll": 0}

        output = location_city_4(maps, player_character, False)
        self.assertEqual(output, (0, False))

    # cannot check text displayed in outside functions
    # @patch('builtins.input', side_effect=['1'])
    # @patch('sys.stdout', new_callable=io.StringIO)
    # def test_location_city_4_found(self, get_output, mock_input):
    #     maps = {"map_visual": [["*", "*", "*", "*", "*"],
    #                            ["*", "*", "*", "*", "*"],
    #                            ["*", "*", "*", "*", "*"],
    #                            ["*", "*", "*", "*", "*"],
    #                            ["*", "*", "*", "*", "*"]],
    #             "map_locations": [["3", "3", "3", "y", "4"],
    #                               ["c", "c", "3", "y", "y"],
    #                               ["2", "2", "c", "3", "3"],
    #                               ["p", "2", "2", "c", "3"],
    #                               ["1", "p", "2", "c", "3"]]}
    #     player_character = {"name": "test",
    #                         "player_position": [4, 0],
    #                         "health": 3,
    #                         "level": 2,
    #                         "exp": 0,
    #                         "add": 0,
    #                         "take_away": 0,
    #                         "re_roll": 0}
    #
    #     location_city_4(maps, player_character, True)
    #     output = get_output.getvalue()
    #     expected_output = "explored"
    #     self.assertIn(expected_output, output)
